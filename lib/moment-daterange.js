 ...
 */
  ...
  function DateRange(start, end, options) {
    // Ensure that "new" isn't required
    if (!(this instanceof DateRange)) {
      if (start in INTERVALS) {
        // The user can pass in an interval string, so we convert to the interval start/stop
        end = moment(this).endOf(start);
        start = moment(this).startOf(start);
      }
...
DateRange.isRange = function(toCheck){
  return toCheck instanceof DateRange;
};
